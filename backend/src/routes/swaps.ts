...
router.patch('/:id/accept', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const swap = await prisma.swapRequest.findFirst({ where: { id: req.params.id, providerId: req.userId, status: 'pending' } });
    if (!swap) { res.status(404).json({ error: 'Swap not found' }); return; }
    const updated = await prisma.swapRequest.update({ where: { id: req.params.id }, data: { status: 'accepted' } });
    res.json(updated);
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.patch('/:id/decline', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updated = await prisma.swapRequest.update({ where: { id: req.params.id }, data: { status: 'declined', escrowHeld: false } });
    res.json(updated);
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.patch('/:id/complete', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const swap = await prisma.swapRequest.findFirst({ where: { id: req.params.id, status: 'accepted', OR: [{ requesterId: req.userId }, { providerId: req.userId }] } });
    if (!swap) { res.status(404).json({ error: 'Swap not found' }); return; }
    ...
    const updated = await prisma.swapRequest.update({ where: { id: req.params.id }, data: { status: 'completed', escrowHeld: false } });
    res.json(updated);
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
